...
public record VideoDetails(
    ...
    public Video getVideo(String videoId) throws IOException {
        YouTube.Videos.List request = this.youtube.videos()
    ...
    return new VideoDetails(video.getId(), title, description, viewCount, likeCount, duration, numberOfCaptions, captionLanguages);
}
...
public List<CaptionInfo> getCaptionInfo(String videoId) throws IOException {
    ...
    return captions.stream()
        .map(caption -> new CaptionInfo(
            caption.getId(),
            caption.getSnippet().getLanguage(),
            caption.getSnippet().getTrackKind(),
            caption.getSnippet().getIsDraft(),
